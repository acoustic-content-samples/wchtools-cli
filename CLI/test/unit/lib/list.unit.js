...
class ListUnitTest extends UnitTest {
    ...
    testListModified (helper, switches, itemName1, itemName2, badItem) {
        describe("CLI-unit-listing", function () {
            it("test list local modified working (ready items only, by default)", function (done) {
            ...
            it("test list local modified working (all items)", function (done) {
                ...
                // Execute the command to list the items to the download directory.
                let error;
                toolsCli.parseArgs(['', UnitTest.COMMAND, "list", switches, "--ready", "--draft", "--mod", "--user", "foo", "--password", "password", "--url", "http://foo.bar/api"])
                    .then(function (msg) {
                        // Verify that the stubs were called as expected, and the expected message was returned.
                        if (switches.includes("--sites")) {
                            // Sites filtered to those in context.siteList.
            ...
            it("test list local modified working (ready items)", function (done) {
                const stubList = sinon.stub(helper._fsApi, "listNames");
                stubList.resolves([{name: itemName1, id: "foo", path: itemName1, status: "ready"}, {
                    name: itemName2,
                    id: "bar",
            ...
            it("test list remote modified working (ready items only, by default)", function (done) {
            ...
            it("test list remote modified working (ready items)", function (done) {
                const stubList = sinon.stub(helper._restApi, "getModifiedItems");
                stubList.resolves([{name: itemName1, id: "foo", path: itemName1, status: "ready"}, {
                    name: itemName2,
                    id: "bar",
            ...
            it("test list remote modified working (draft items)", function (done) {
                ...
                toolsCli.parseArgs(['', UnitTest.COMMAND, "list", switches, "--draft", "--server", "--mod", "--dir", ARTIFACTS_DIR, "--user", "foo", "--password", "password", "--url", "http://foo.bar/api"])
                    .then(function (msg) {
                        ...
                        if (switches.includes("--sites")) {
                            // Sites filtered to those in context.siteList.
                            expect(stubHashes).to.have.been.calledTwice;
                            expect(msg).to.contain('artifacts listed 1');
                        } else {
                            expect(msg).to.contain('artifacts listed 2');
                        }
    ...
    testListAll (helper, switches, itemName1, itemName2, badItem) {
        describe("CLI-unit-listing", function() {
            ...
            it("test list local working (all items)", function (done) {
                ...
                // Execute the command to list the items to the download directory.
                let error;
                toolsCli.parseArgs(['', UnitTest.COMMAND, "list", switches, "--ignore-timestamps", "--ready", "--draft", "-q", "--user", "foo", "--password", "password", "--url", "http://foo.bar/api"])
                    .then(function (msg) {
                        // Verify that the stub was called once, and that the expected message was returned.
                        if (switches.includes("--sites")) {
                            // Sites filtered to those in context.siteList.
            ...
            it("test list local working (ready items)", function (done) {
                const stub = sinon.stub(helper._fsApi, "listNames");
                stub.resolves([{name: itemName1, id: "foo", path: itemName1, status: "ready"}, {
                    name: itemName2,
                    id: "bar",
            ...
            it("test list remote working (ready items only, by default)", function (done) {
                let stubGet;
                if (switches === "--sites") {
            ...
            it("test list remote working (all items)", function (done) {
                ...
                // Execute the command to list the items to the download directory.
                let error;
                toolsCli.parseArgs(['', UnitTest.COMMAND, "list", switches, "--server", "--ignore-timestamps", "--ready", "--draft", "--user", "foo", "--password", "password", "--url", "http://foo.bar/api"])
                    .then(function (msg) {
            ...
            it("test list remote working (ready items)", function (done) {
                let stubGet;
                if (switches === "--sites") {
                    // Remove the global stub and create a local SitesREST.getItems stub that returns the standard sites
                    // the first time (initSites) then returns the test values the second time (the actual list).
